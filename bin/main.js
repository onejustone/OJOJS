(function (global, factory) {
  ...
  function getTarget(event) {
    return event.target || event.srcElement; // target 现代浏览器
    // srcElement IE8 以下
  }

  function preventDefault(event) {
    if (event.preventDefault) {
      event.preventDefault(); // 阻止现代浏览器默认事件
    } else {
      event.returnValue = false; // 阻止 IE8 默认事件
    }
  }
  ...
  function stopPropagation(event) {
    ...
    if (event.stopPropagation) {
      event.stopPropagation(); // 阻止现代浏览器事件传播
    } else {
      event.cancelBubble = true; // 阻止 IE8 事件传播
    }
  }
  ...
  function getWheelDelta() {
    ...
    if (event.wheelDelta) {
      ...
      return event.wheelDelta; // return (client.engine.opera && client.engine.opera < 9.5 ? -event.wheelDelta : event.wheelDelta);
    } else {
  ...
  function getPageCoordinates(event) {
    ...
    var pageY = event.pageY; // IE8 不支持事件对象上的页面坐标，所以需要通过 client 偏移量 和 scroll 偏移量来计算
  ...
  function popupCenterWindow(href, title) {
    var percent = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0.8;
    var closeCb = arguments.length > 3 ? arguments[3] : undefined;
    var w = window.screen.width * percent;
    var h = window.screen.height * percent;
    var left = window.screen.width * (1 - percent) / 2;
    var top = window.screen.height * (1 - percent) / 2;
    var windowHandler = window.open(href, title, "\n    toolbar=no,\n    location=no,\n    directories=no,\n    status=no,\n    menubar=no,\n    scrollbars=no,\n    resizable=no,\n    copyhistory=no,\n    width=".concat(w, ",\n    height=").concat(h, ", top=").concat(top, ", left=").concat(left));
  ...
  function formartFileSize(size) {
    ...
    return "".concat(estimateSize).concat(unit);
  }
  var format = {
    formatNumebr: formatNumebr,
  ...
  function sum() {
    ...
    return flatArgumentsArr.reduce(function (x, y) {
  ...
  var isEmptyItem = function _isEmptyItem(values) {
    return values.every(function (child) {
      return isEmptyArray(child.value) || isEmpty(child.value);
    });
  };
  ...
  var isObject = function _isObject(value) {
    return value && _typeof_1(value) === 'object' && value.constructor === Object;
  };
  ...
  var _typeof = function _typeof(obj) {
    ...
    'Boolean String Number Array Function Object Null Date RegExp Error'.split(' ').forEach(function (e, i) {
      class2type["[object ".concat(e, "]")] = e.toLowerCase();
    });
    return _typeof_1(obj) === 'object' || typeof obj === 'function' ? class2type[class2type.toString.call(obj)] : _typeof_1(obj);
  };

  var each = function _each(obj, callback) {
    var i, len;
  ...
  var type = {
    ...
    isString: isString,
    isNativeStringType: isNativeStringType,
    _typeof: _typeof,
  ...
  var getValuePropByPath = function _getValuePropByPath(obj, path, strict) {
  ...
  var deepCloneObj = function _deepCloneObj(source) {
    ...
    if (!source && _typeof_1(source) !== 'object') {
    ...
    for (var keys in source) {
      if (source.hasOwnProperty(keys)) {
        if (source[keys] && _typeof_1(source[keys]) === 'object') {
  ...
  var shortcut = {
    resetObjectToEmpty: resetObjectToEmpty,
    getValuePropByPath: getValuePropByPath,
  ...
  function humanize(timestamp) {
    ...
    var deltaDay = Math.floor((timestamp - now.clone().startOf('day').unix()) / secondsOf1D);
    var time = target.format('HH:mm');

    if (!target.isValid() || timestamp < 0) {
      return '';
    } else if (isSameOf(now, target, 'day')) {
      // 今天
      return '今天'; // return target.format('HH:mm')
    } else if (Math.abs(deltaDay) < 7) {
      // 7 天內;
      var date = "".concat(deltaDay, "\u5929\u540E");
      ...
      if (deltaDay === 2) {
        ...
        date = "".concat(Math.abs(deltaDay), "\u5929\u524D");
      }

      return addSuffix(date, time, mode);
    } else if (isSameOf(now, target, 'year')) {
      // 今年
      var _date = target.format('MM-DD');

      return addSuffix(_date, time, mode);
    } else {
      // 更远
  ...
  var index = objectSpread({}, dom, energy, env, format, operator, other, shortcut, time, type);
  ...
  exports.isEmptyItem = isEmptyItem;
